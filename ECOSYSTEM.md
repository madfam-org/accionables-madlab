...
| **Selva** | `madfam-org/selva-office` (`autoswarm-office` former name) | LLM inference routing + agent orchestration |
...
- **Inference**: every LLM call should route through Selva
  (`selva-office`; former repo name `autoswarm-office`) at `/v1` (OpenAI-compatible). Do not talk directly
...
### Day-to-day for madlab-client

The commands below default to `madlab-client` — the primary service name for
...
enclii ps --wide
enclii ps madlab-client --env production

# Logs (tail, filter, history)
enclii logs madlab-client -f                          # live tail
enclii logs madlab-client --since 1h --level error    # last hour, errors only
enclii logs madlab-client --env staging -f
...
# Rollback
enclii rollback madlab-client                         # previous release
enclii rollback madlab-client --to-revision 5

# Releases + history
enclii releases madlab-client                          # list builds
enclii releases madlab-client --latest --output json

# Secrets (routed through Lockbox → Vault → ESO → K8s)
enclii secrets list madlab-client
enclii secrets set MY_KEY=value --service madlab-client --secret
enclii secrets rm MY_KEY --service madlab-client

# Domains, tunnel routes, DNS
enclii domains list madlab-client
enclii domains add madlab-client my.example.com       # auto-provisions tunnel route + DNS
...
# Routing (ingress + TLS)
enclii junctions list madlab-client
